// Shamelessly copies from https://docs.rs/nalgebra/latest/src/nalgebra/linalg/lu.rs.html
// with changes to remove the ComplexField restriction

use nalgebra::{allocator::{Allocator, Reallocator}, constraint::{SameNumberOfRows, ShapeConstraint}, DefaultAllocator, Dim, DimMin, DimMinimum, Field, Matrix, OMatrix, PermutationSequence, Scalar, Storage, StorageMut};

use crate::rat::NumEx;

/// LU decomposition with partial (row) pivoting.
#[derive(Clone, Debug)]
pub struct LU<T: NumEx, R: DimMin<C>, C: Dim>
where
...
impl<T: NumEx, R: DimMin<C>, C: Dim> Copy for LU<T, R, C>
where
    DefaultAllocator: Allocator<R, C> + Allocator<DimMinimum<R, C>>,
    OMatrix<T, R, C>: Copy,
...
pub fn try_invert_to<T: NumEx, D: Dim, S>(
    ...
    DefaultAllocator: Allocator<D, D>,
{
    ...
    for i in 0..dim {
        let piv = matrix.view_range(i.., i).iamax() + i;
...
impl<T: NumEx, R: DimMin<C>, C: Dim> LU<T, R, C>
where
    DefaultAllocator: Allocator<R, C> + Allocator<DimMinimum<R, C>>,
{
...
impl<T: NumEx, D: DimMin<D, Output = D>> LU<T, D, D>
where
    ...
    S: StorageMut<T, R, C>,
{
    ...
    for k in 0..pivot_row.ncols() {
        std::mem::swap(&mut pivot_row[k], &mut down[(piv - 1, k)]);
        down.column_mut(k)
            .axpy(-pivot_row[k].clone(), &coeffs, T::one());
    }
}